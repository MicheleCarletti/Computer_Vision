...
int main(int argc, char **argv)
{
  ...
  while(!exit_loop)
  {
    ...
    /*preparo i dati per eseguire il k-means*/
    std::vector<cv::Vec6f> img_data;
    for (int r = 0; r < image.rows; r++) {
      for (int c = 0; c < image.cols; c++) {
	cv::Vec3f pixel = image.at<cv::Vec3f>(r, c);  //prelevo i 3 canali del pixel
  /*inserisco nel vettore l'informazione del colore (primi 3 elementi) e le coordinate normalizzate di colonne e righe*/
	img_data.push_back(cv::Vec6f(pixel[0], pixel[1], pixel[2], float(c)/image.cols*255, float(r)/image.rows*255, 0));
	//std::cout << pixel[0] << " "; //stampa dei pixel canale blu 
      }
    }
